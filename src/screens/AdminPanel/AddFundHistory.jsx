...
import { getAllAddFundHistory } from "../../api/admin.api";

const AddFundHistory = () => {
  const [allAddFundHistory, setAllAddFundHistory] = useState([]);
  const dispatch = useDispatch();
  const location = useLocation();
  const data = location?.state;
  const fetchAllAddFundHistory = async () => {
    try {
      dispatch(setLoading(true));
      const response = await getAllAddFundHistory();
      setAllAddFundHistory(response?.data);
    } catch (err) {
      console.log(err);
    } finally {
  ...
  useEffect(() => {
    fetchAllAddFundHistory();
  }, []);

  const filteredIncomeHistory =
    data === "today"
      ? allAddFundHistory.filter((item) => isToday(new Date(item.createdAt)))
      : allAddFundHistory;
  ...
  const columns = [
    ...
    },
    {
      ...
      searchValue: (row) => row?.user?.username?.toString() || "",
    },
    {
    ...
    },
    {
      ...
      searchValue: (row) => row?.investment?.toString() || "",
    },
    {
      header: "Percentage",
      accessor: "percentage",
