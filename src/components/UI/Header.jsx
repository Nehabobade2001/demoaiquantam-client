    ...
    <div className="sticky top-0 z-50 relative bg-[var(--bg)] text-[var(--text)] dark:bg-[var(--bg)] dark:text-[var(--text)] shadow-lg dark:shadow-md dark:shadow-gray-600">
        ...
        <div className="flex items-center justify-around space-x-4 2xl:space-x-12">
          <Link to="/" data-discover="true" className="flex items-center gap-3">
            <img
              ...
              alt="Logo"
              className="h-14 w-14 cursor-pointer p2 bg-black dark:bg-transparent rounded"
            />
          </Link>
          ...
          <div className="hidden lg:flex lg:space-x-8 lg:pl-8 xl:space-x-16">
            {navItems.map((menu, index) => (
              <div className="relative group" key={index}>
                <Link
                  className={`text-lg font-medium hover:text-[var(--text)] transition-colors duration-200 ${isActive(menu.href)
      ...
      <div className="flex flex-row justify-between items-center lg:hidden py-4 px-4">
        <Link to="/" data-discover="true" className="flex items-center gap-2">
          <img
            src={MainContent.shortLogo}
            alt="Logo"
            className="h-10 w-auto cursor-pointer"
          />
          <span className="text-lg font-bold text-[var(--text)]">AIQuantum Crypto</span>
        </Link>
      ...
      <div
        ref={mobileMenuRef}
        className={`lg:hidden fixed top-0 left-0 w-3/4 h-full z-50 bg-[var(--bg)] text-[var(--text)] shadow-xl px-6 py-8 space-y-4 overflow-auto hide-scrollbar ${isMobileMenuOpen
        ...
        {navItems.map((menu, idx) => (
          <div key={idx}>
            <Link
              ...
              className={`block py-2 text-lg font-semibold ${isActive(menu.href)
                  ? "text-[var(--btn-hover-text)] underline"
                  : ""
                }`}
            >
